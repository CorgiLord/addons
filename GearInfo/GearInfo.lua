...
function update()
	...
	if windower.ffxi.get_info().logged_in == false then
		...
		if settings.player.show_total_haste ==  true then
			if not sections.block[1] then sections.block[1] = ImageBlock.New(2,'block','green', 'Gear.H', 00) end
			if not sections.block[2] then sections.block[2] = ImageBlock.New(3,'block','green', 'Magic.H', 00) end
			--if not sections.block[3] then sections.block[3] = ImageBlock.New(4,'block','green', 'JA.H', 00) end
			if not sections.block[3] then sections.block[3] = ImageBlock.New(4,'block','green', 'Total.H', 00) end
			...
			windower.text.set_text(sections.block[3].text[2].name,  Total_haste)
		...
		if settings.player.show_tp_Stuff == true then 
			...
			if not sections.block[9] then sections.block[9] = ImageBlock.New(10,'block','orange', 'TP/h', 00) end
			if not sections.block[10]  then sections.block[10] = ImageBlock.New(11,'block','orange', 'to WS', 00) end
			...
			if WSTP > 0 then
				if not sections.block[11] then sections.block[11] = ImageBlock.New(12,'block','orange', 'aft WS', 00) end
		...
		if settings.player.show_acc_Stuff == true then
			...
			if Total_acc.sub > 0 then
				if not sections.block[12] then
					sections.block[12] = ImageBlock.New(13,'block','blue', 'Acc.1', 00)
				end
				if not sections.block[13] then
					sections.block[13] = ImageBlock.New(14,'block','blue', 'Acc.2', 00)
				end
				...
				if not sections.block[12] then
					sections.block[12] = ImageBlock.New(13,'block','blue', 'Acc.', 00)
				end
				windower.text.set_text(sections.block[12].text[2].name, Total_acc.main)
		...
		if settings.player.show_att_Stuff == true then
			...
			if Total_att.sub > 0 then
				if not sections.block[14] then
					sections.block[14] = ImageBlock.New(15,'block','red', 'Att.1', 00)
				end
				if not sections.block[15] then
					sections.block[15] = ImageBlock.New(16,'block','red', 'Att.2', 00)
				end
				...
				if not sections.block[14] then
					sections.block[14] = ImageBlock.New(15,'block','red', 'Att.', 00)
				end
				windower.text.set_text(sections.block[14].text[2].name, Total_att.main)
		...
		if settings.player.show_DW_Stuff == true then
			...
			if player.equipment.sub.category == 'Weapon' then 
				if player.equipment.sub.damage then
					...
					if not sections.block[24] then sections.block[24] = ImageBlock.New(25,'block','dark-blue', 'DW', 00) end
					if not sections.block[25] then sections.block[25] = ImageBlock.New(26,'block','dark-blue', 'Needed', 00) end
		...
		if settings.player.show_STP == true then
			if not sections.block[4] then sections.block[4] = ImageBlock.New(5,'block','orange', 'STP', 00) end
			windower.text.set_text(sections.block[4].text[2].name, Gear_info['Store TP'] + Buffs_inform['Store TP'])
		else
			if sections.block[4] then sections.block[4]:delete() end
		end		
